...
class multiplicative_attention_model(tf.keras.Model):
	...
	```python
		...
		_ = model(tf.ones(shape=(1,20)))

		# Take a look at model
		model.summary()
	```
	"""
	def __init__(self, input_shape, input_vocab_size, output_vocab_size, name="Additive-Attention", input_embed_dim=200, hidden_state_dim=200, alignment_dim=40, **kwargs):
		...
		self._decoder_state_array = tf.TensorArray(dtype=tf.float32,size=self._n_step)
	...
	def call(self, inputs, training=True):
		...
		encoder_sequence, forward_state, backward_state = self._bi_gru_encoder_layer(inputs_embed)
		...
		for step in range(self._n_step):
			...
			decoder_hidden_state, _ = self._decoder_cell(inputs=decoder_input, states=[decoder_hidden_state])
